...
def sshd_remedy(key: str, value: str, risk: str = "medium") -> Remedy:
    # A bare `no`/`yes` maps to a Nix bool; anything else is a quoted Nix string.
    # (Kept out of the f-string to stay valid on Python < 3.12, which rejects a
    # backslash inside an f-string expression.)
    nix_val = "false" if value == "no" else f'"{value}"'
    return Remedy(
        ...
        nixos_note=f"services.openssh.settings.{key} = {nix_val};",
        nix=(f"services.openssh.settings.{key}", nix_val),
    )
